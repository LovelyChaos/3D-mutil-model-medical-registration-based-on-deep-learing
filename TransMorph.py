    ...
    def __init__(self,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(
                ...
                shift_size=(0, 0, 0) if (i % 2 == 0) else (window_size[0] // 2, window_size[1] // 2, window_size[2] // 2),#将两个不同的transformer组合在一起，并把将这个组合按照depth循环
                mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias,
                qk_scale=qk_scale,
                rpe=rpe,
...
class TransMorph_CopyX2(nn.Module):
    def __init__(self, config):
        '''
        TransMorph Model
        '''
        super(TransMorph_CopyX2, self).__init__()
        if_convskip = config.if_convskip
        self.if_convskip = if_convskip
        if_transskip = config.if_transskip
        self.if_transskip = if_transskip
